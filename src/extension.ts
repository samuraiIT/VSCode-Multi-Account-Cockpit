...
interface CockpitToolsIndexEntry {
    id: string;
    email: string;
}

interface CockpitTokenItem {
    email: string;
    refresh_token: string;
}

interface CockpitAntigravitySyncResult {
    entriesInIndex: number;
    validTokenItems: number;
    imported: number;
    skipped: number;
}

function collectCockpitToolsAntigravityTokenItems(sharedDir: string): { entries: CockpitToolsIndexEntry[]; tokenItems: CockpitTokenItem[] } {
    const accountsIndexPath = path.join(sharedDir, 'accounts.json');
    const accountsDirPath = path.join(sharedDir, 'accounts');

    let indexData: { accounts?: CockpitToolsIndexEntry[] };
    try {
        const raw = fs.readFileSync(accountsIndexPath, 'utf8');
        indexData = JSON.parse(raw) as { accounts?: CockpitToolsIndexEntry[] };
    } catch {
        throw new Error('Could not read ~/.antigravity_cockpit/accounts.json');
    }

    const entries = indexData.accounts ?? [];
    const tokenItems: CockpitTokenItem[] = [];
    ...
    return { entries, tokenItems };
}

async function importAntigravityFromCockpitTools(
    tokenItems: CockpitTokenItem[],
    progressTitle: string,
    showProgress: boolean,
): Promise<{ imported: number; skipped: number }> {
    const runImport = async (
        report: (current: number, total: number, email: string) => void,
        cancellationToken: { cancelled: boolean },
    ) => {
        return antigravityToolsSyncService.importFromJson(
            JSON.stringify(tokenItems),
            (current, total, email) => report(current, total, email),
            cancellationToken,
        );
    };

    if (!showProgress) {
        const result = await runImport(() => undefined, { cancelled: false });
        const skipped = result.skipped.filter(s => s.reason !== 'user_cancelled').length;
    ...
    return vscode.window.withProgress(
        ...
        async (progress, token) => {
            ...
            token.onCancellationRequested(() => { cancellationToken.cancelled = true; });

            const result = await runImport((current, total, email) => {
                progress.report({
                    message: `${email} (${current}/${total})`,
                    increment: Math.max(1, Math.round(100 / Math.max(1, total))),
                });
            }, cancellationToken);
            const skipped = result.skipped.filter(s => s.reason !== 'user_cancelled').length;
            return { imported: result.imported, skipped };
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
        ...
        vscode.commands.registerCommand('agCockpit.autoImportCockpitAccounts', async (options?: { silent?: boolean }) => {
            ...
            try {
                ...
                if (tokenItems.length > 0) {
                    antigravityImport = await importAntigravityFromCockpitTools(
                        tokenItems,
                        'Auto-importing Antigravity accounts from Cockpit Tools…',
                        !silent,
                    );
                }
                ...
                logger.error(`[autoImportCockpitAccounts] ${err}`);
                if (!silent) {
                    vscode.window.showErrorMessage(`Cockpit Tools auto-import failed: ${err}`);
                }
            }
        }),
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agCockpit.syncFromCockpitTools', async () => {
            const sharedDir = getCockpitToolsSharedDir();
            let syncResult: CockpitAntigravitySyncResult | null = null;
            ...
            try {
                ...
                const imported = await importAntigravityFromCockpitTools(
                    tokenItems,
                    'Syncing Antigravity accounts from Cockpit Tools…',
                    true,
                );
                syncResult = {
                    entriesInIndex: entries.length,
                    validTokenItems: tokenItems.length,
                    imported: imported.imported,
                    skipped: imported.skipped,
                };
            } catch (error) {
                const err = error instanceof Error ? error.message : String(error);
                logger.error(`[syncFromCockpitTools] ${err}`);
                vscode.window.showErrorMessage(`Sync from Cockpit Tools failed: ${err}`);
                return;
            }
            ...
            if (syncResult.imported > 0) {
                void vscode.commands.executeCommand('agCockpit.accountTree.refresh');
                const parts = [`Synced ${syncResult.imported} account(s)`];
                if (syncResult.skipped > 0) {
                    parts.push(`${syncResult.skipped} skipped`);
                }
            ...
            vscode.window.showInformationMessage(
                `Sync from Cockpit Tools: all ${syncResult.entriesInIndex} account(s) already up to date.`,
            );
        }),
    );
    ...
    const autoImportOnStartup = vscode.workspace
        .getConfiguration('agCockpit')
        .get<boolean>('cockpitToolsAutoImportOnStartup', true);
    if (autoImportOnStartup) {
        setTimeout(() => {
            void vscode.commands.executeCommand('agCockpit.autoImportCockpitAccounts', { silent: true });
        }, 3500);
    }
